...
def C5S3_dmg_example(
    random_seed=114514,
    dir=r"./results/postprocessed",
    type="csv",
    loc="C5S3",
    name="C5S3-MS-FNO_SignedError+Intact",
    n_elem=540,
...
for n in [10, 15, 20]:
    ...
    for case_name, pred_bin in cases.items():
        fig_name = f"C4S3_confmat_{case_name}_c80_bin={n_bins}.png"
        fig_path = os.path.join(output_dir, "C5S3", "fig")
        os.makedirs(fig_path, exist_ok=True)
        acc, prec, rec, f1 = C5S3_plot_cm(
            true_labels, pred_bin, n,
    ...
    csv_path = os.path.join(output_dir, "C5S3", "csv")
    os.makedirs(csv_path, exist_ok=True)
    metrics_df80.to_csv(os.path.join(csv_path, f"C4S3_main_metrics_c80_bin={n_bins}.csv") )
