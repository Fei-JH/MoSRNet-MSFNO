...
def train_1d(config,
    ...
    for ep in epochs:
        ...
        for down, gt, dmg, M, K in train_iter:
            if use_tqdm:
                train_t = time.time()
            down, gt, dmg, M, K = [t.to(device) for t in (down, gt, dmg, M, K)]
            ...
            for idx, batch_evaluation in enumerate(batch_evaluations):
                ...
                if evaluation_name in ["TVLoss"]:
                    # TVLoss: 2D tensor, shape (batch, -1)
                    t_evaluation = batch_evaluation(out.view(out.shape[0], -1), gt.view(gt.shape[0], -1))
                elif evaluation_name in ["MOLoss", "MOLoss_AIRM"]:
                    # MassOrthogonalityLoss: (mode, M)
                    t_evaluation = batch_evaluation(out, M)
                elif evaluation_name in ["SOLoss", "SOLoss_AIRM"]:
                    # StiffnessOrthogonalityLoss: (mode, K)
                    t_evaluation = batch_evaluation(out, K)
                elif evaluation_name in ["SCLoss"]:
                    # StiffnessConsistencyLoss: (mode_pred, k_true)
                    t_evaluation = batch_evaluation(out, dmg)
                else:
                    # Default case for other losses
                    t_evaluation = batch_evaluation(out.view(out.shape[0], -1), gt.view(gt.shape[0], -1))
            ...
            for idx, batch_loss in enumerate(batch_losses):
                ...
                if loss_name in ["TVLoss"]:
                    # TVLoss: 2D tensor, shape (batch, -1)
                    t_loss = batch_loss(out.view(out.shape[0], -1))
                elif loss_name in ["MOLoss", "MOLoss_AIRM"]:
                    # MassOrthogonalityLoss: (mode, M)
                    t_loss = batch_loss(out, M)
                elif loss_name in ["SOLoss", "SOLoss_AIRM"]:
                    if ep < 20:
                        t_loss = 0
                    else:
                       # StiffnessOrthogonalityLoss: (mode, K)
                        t_loss = batch_loss(out, K)
                elif loss_name in ["SCLoss"]:
                    # StiffnessConsistencyLoss: (mode_pred, k_true)
                    t_loss = batch_loss(out, dmg)
                else:
                    # Default case for other losses
        ...
        with torch.no_grad():
            ...
            for down, gt, dmg, M, K in valid_iter:
                if use_tqdm:
                    valid_t = time.time()
                down, gt, dmg, M, K = [t.to(device, non_blocking=True) for t in (down, gt, dmg, M, K)]
                out = model(down)
                ...
                for idx, batch_evaluation in enumerate(batch_evaluations):
                    evaluation_name = batch_evaluation.__class__.__name__
                    if evaluation_name in ["TVLoss"]:
                        v_evaluation = batch_evaluation(out.view(out.shape[0], -1))
                    elif evaluation_name in ["MOLoss", "MOLoss_AIRM"]:
                        v_evaluation = batch_evaluation(out, M)
                    elif evaluation_name in ["SOLoss", "SOLoss_AIRM"]:
                        v_evaluation = batch_evaluation(out, K)
                    elif evaluation_name in ["SCLoss"]:
                        v_evaluation = batch_evaluation(out, dmg)
                    else:
                ...
                for idx, batch_loss in enumerate(batch_losses):
                    loss_name = batch_loss.__class__.__name__
                    if loss_name in ["TVLoss"]:
                        v_loss = batch_loss(out.view(out.shape[0], -1))
                    elif loss_name in ["MOLoss", "MOLoss_AIRM"]:
                        v_loss = batch_loss(out, M)
                    elif loss_name in ["SOLoss", "SOLoss_AIRM"]:
                        v_loss = batch_loss(out, K)
                    elif loss_name in ["SCLoss"]:
                        v_loss = batch_loss(out, dmg)
                    else:
